...
public class SalleRepositoryImpl implements SalleRepository {
    ...
    @Override
    public List<Salle> findSallesDispo(LocalDateTime start, LocalDateTime end) {
        // Requête JPQL pour trouver les salles disponibles
        String jpql = "SELECT DISTINCT s FROM Salle s WHERE s.id NOT IN (SELECT r.salle.id FROM Reservation r " +
    ...
    @Override
    public List<Salle> findAllPagination(int page, int size) {
        // Pagination avec JPA
